...
THNet is a Python 3.11 software designed to infer HLA haplotypes from T-cell beta chain repertoire datasets that can calculate mismatch scores (MS) by taking the HLA allele compositions of both donor and recipient as input to predict the outcome of transplantation. The model can infer 208 HLA alleles based on the T-cell beta chain repertoire, with higher accuracy for common alleles. 
...
```
An example of the input files for Mismatch_score can be referred at `THNet/HLA_inference/example/input_example.csv`
...
An example of the input files for Mismatch_score can be referred at `THNet/Mismatch_score/example/input_example.csv`
